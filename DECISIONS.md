...
| **Server/Client auth component split** | Auth pages are server components (page.tsx) with client form components (LoginForm, SignupForm) as children. Follows AGENTS.md Rule 10 — pages stay server components, interactivity extracted to leaf client components. | **Accepted.** |
...
| **Pre-Phase 2 stabilization** | Ran full cleanup pass before editor work. | **Accepted.** Deleted unused stub components (ProblemTable, ProblemSearch). Verified route ownership. Removed duplicate auth guards. |
